...
double Raven_Feature::TotalWeaponStrength(Raven_Bot* pBot)
{
  ...
  double NumBombs = (double)pBot->GetWeaponSys()->GetAmmoRemainingForWeapon(type_grenade);
  ...
  return Tweaker + (1-Tweaker)*(NumSlugs + NumCartridges + NumRockets + NumBombs)/(TotalRoundsCarryable);
}
